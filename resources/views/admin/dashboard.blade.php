<x-layouts::auth>
    ...
    <div class="fixed inset-0 w-full h-full overflow-y-auto bg-[#0C4D8B] font-['Source_Sans_3'] text-white z-50">
        ...
        <header class="flex justify-between items-center px-4 sm:px-8 md:px-12 py-5 sm:py-7 md:py-8 w-full">
            <div>
                <img src="{{ asset('images/Logo_1.svg') }}" alt="NovaBank" class="w-[140px] sm:w-[170px] md:w-[200px] h-auto">
            </div>
            
            {{-- Icono de Usuario Circular con Dropdown --}}
            <div class="relative" x-data="{ open: false }">
                <button @click="open = !open" class="w-[42px] h-[42px] sm:w-[46px] sm:h-[46px] md:w-[50px] md:h-[50px] bg-[#3B82F6] rounded-full flex items-center justify-center cursor-pointer shadow-lg hover:bg-blue-600 transition-colors focus:outline-none">
                    <svg class="w-6 h-6 sm:w-7 sm:h-7 md:w-8 md:h-8 text-white mt-0.5 sm:mt-1" fill="currentColor" viewBox="0 0 24 24">
                        <path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z"/>
                    </svg>
                </button>

                {{-- Dropdown Menu --}}
                <div x-show="open" @click.away="open = false" x-transition:enter="transition ease-out duration-150" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100" x-transition:leave="transition ease-in duration-100" x-transition:leave-start="opacity-100 scale-100" x-transition:leave-end="opacity-0 scale-95" class="absolute right-0 mt-3 w-44 sm:w-48 bg-[#072b4e] rounded-xl shadow-2xl border border-white/10 overflow-hidden z-50" style="display: none;">
        ...
        {{-- CONTENIDO CENTRAL --}}
        <main class="flex flex-col items-center mt-2 sm:mt-3 md:mt-4 px-4 sm:px-6 w-full" x-data="{
            showAreaModal: false,
            areaId: null,
            areaLabel: '',
            areaOperador: '',
            ...
            {{-- Título Principal --}}
            <h1 class="text-[20px] sm:text-[24px] md:text-[26px] font-bold tracking-wider uppercase mb-8 sm:mb-10 md:mb-12 text-center w-full">
                Panel Administrador
            </h1>

            {{-- Grid de Ventanillas --}}
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6 md:gap-8 w-full max-w-[1000px] mb-10 sm:mb-12">
            ...
            {{-- Grid de Asesores --}}
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 md:gap-8 w-full max-w-[1000px] mb-10 sm:mb-12">
                @if($asesores->count() > 0)
            ...
            <template x-teleport="body">
                <div x-show="showAreaModal"
                    ...
                    <div x-show="showAreaModal"
                         ...
                         class="relative bg-[#02B48A] border border-white/15 rounded-3xl shadow-2xl p-6 sm:p-8 md:p-10 max-w-md w-full mx-4 text-center font-['Source_Sans_3']">
                        ...
                        <form x-ref="areaForm" method="POST" :action="areaActive ? '/admin/area/' + areaId + '/disable' : '/admin/area/' + areaId + '/enable'">
                            ...
                            <template x-if="areaActive">
                                <button type="submit"
                                        class="bg-[#CC0000] hover:bg-[#A30000] text-white px-7 sm:px-8 py-3.5 sm:py-4 rounded-2xl font-bold text-[16px] sm:text-[18px] transition-all active:scale-95 shadow-xl w-full sm:min-w-[220px]">
                                    Deshabilitar Área
                                </button>
                            </template>
                            <template x-if="!areaActive">
                                <button type="submit"
                                        class="bg-[#02854d] hover:bg-[#026e3f] text-white px-7 sm:px-8 py-3.5 sm:py-4 rounded-2xl font-bold text-[16px] sm:text-[18px] transition-all active:scale-95 shadow-xl w-full sm:min-w-[220px]">
                                    Habilitar Área
                                </button>
                            </template>
                        </form>
            ...
            {{-- Botones de Acción --}}
            <div class="flex flex-col md:flex-row gap-4 sm:gap-6 md:gap-8 w-full max-w-[800px] justify-center mb-16 sm:mb-20 md:mb-24" x-data="{ showConfirm: false }">
                
                {{-- Botón Encender/Apagar Generación de Turnos --}}
                <form method="POST" action="{{ route('admin.turns.toggle') }}" x-ref="toggleForm">
                    @csrf
                    @if($isActive)
                        <button type="button" class="bg-[#CC0000] hover:bg-[#A30000] text-white px-6 sm:px-8 md:px-10 py-4 sm:py-5 rounded-2xl font-bold text-[16px] sm:text-[17px] md:text-[18px] transition-all active:scale-95 shadow-xl text-center leading-tight w-full sm:min-w-[280px]"
                                @click="showConfirm = true">
                            Apagar Generación <br> de Turnos
                        </button>
                    @else
                            <button type="submit" class="bg-[#02B48A] hover:bg-[#029A73] text-white px-6 sm:px-8 md:px-10 py-4 sm:py-5 rounded-2xl font-bold text-[16px] sm:text-[17px] md:text-[18px] transition-all active:scale-95 shadow-xl flex items-center justify-center w-full sm:min-w-[280px] text-center">
                ...
                <template x-teleport="body">
                    <div x-show="showConfirm"
                        ...
                        <div x-show="showConfirm"
                             ...
                             x-transition:leave-start="opacity-100 scale-100"
                             x-transition:leave-end="opacity-0 scale-90"
                             class="relative bg-[#0C4D8B] border border-white/15 rounded-3xl shadow-2xl p-6 sm:p-8 md:p-10 max-w-md w-full mx-4 text-center font-['Source_Sans_3']">
                            ...
                            {{-- Texto --}}
                            <h2 class="text-white text-[20px] sm:text-[22px] font-bold mb-3">¿Estás seguro?</h2>
                            <p class="text-white/80 text-[15px] sm:text-[16px] leading-relaxed mb-6 sm:mb-8">
                ...
                {{-- Botón Dar Alta --}}
                <a href="/admin/registro-empleado" class="bg-[#02B48A] hover:bg-[#029A73] text-white px-6 sm:px-8 md:px-10 py-4 sm:py-5 rounded-2xl font-bold text-[16px] sm:text-[17px] md:text-[18px] transition-all active:scale-95 shadow-xl flex items-center justify-center w-full sm:min-w-[280px] text-center">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-5 sm:gap-7 md:gap-8 w-full max-w-[1000px] mb-10 sm:mb-12 text-center">
                <div>
                    <h3 class="text-[14px] sm:text-[15px] md:text-[16px] font-bold leading-tight mb-2 sm:mb-3">Total de turnos<br>Generados:</h3>
                    <p class="text-[24px] sm:text-[26px] md:text-[28px] font-bold">{{ $totalGenerados }}</p>
                </div>
                <div>
                    <h3 class="text-[14px] sm:text-[15px] md:text-[16px] font-bold leading-tight mb-2 sm:mb-3">Total de turnos<br>Expirados:</h3>
                    <p class="text-[24px] sm:text-[26px] md:text-[28px] font-bold">{{ $totalExpirados }}</p>
                </div>
                <div>
                    <h3 class="text-[14px] sm:text-[15px] md:text-[16px] font-bold leading-tight mb-2 sm:mb-3">Total de turnos<br>Cancelados:</h3>
                    <p class="text-[24px] sm:text-[26px] md:text-[28px] font-bold">{{ $totalCancelados }}</p>
                </div>
                <div>
                    <h3 class="text-[14px] sm:text-[15px] md:text-[16px] font-bold leading-tight mb-2 sm:mb-3">Total de turnos<br>Finalizados:</h3>
                    <p class="text-[24px] sm:text-[26px] md:text-[28px] font-bold">{{ $totalFinalizados }}</p>
                </div>
            </div>
